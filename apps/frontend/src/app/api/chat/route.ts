...
export async function POST(req: NextRequest) {
    ...
    const result = await streamText({
        ...
        system: [
            ...
            "Your job is to:",
            "1) Offer trigger choices (fetch tools first). For webhook then ask for the json body for request body, ask for direct body in json or directly ask field then construct json and show user and say this will be if user agrees move ahead. Also dont ask for any other trigger if user already told you which trigger to use, you can ask for the fields of webhook that is needed, also ask if user wants dummy data for the trigger for the fields to be autogenerated",
            "2) Offer action choices (fetch tools next). Don't list the options if user already told you which trigger or action to use you can ask the metdata.",
            ...
            "5) Do not ask for any metadata related to the Webhook trigger itself; users only provide data for action metadata, or also give option to use dummy data for the trigger for the fields to be autogenerated",
            "6) Before creating ask if user wants to add more actions or not if not then confirm with user then Call createZap with { availableTriggerId, triggerMetadata, actions:[{ availableActionId, actionMetadata }] }.",
